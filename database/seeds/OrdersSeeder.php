    ...
    public function run()
    {
        ...
        $products->unique('id')->each(function (\App\Models\Product $product) {
            ...
            $result = \App\Models\OrderItem::query()
                ...
                ->first([
                    \Illuminate\Support\Facades\DB::raw('count(*) as review_count'),
                    \Illuminate\Support\Facades\DB::raw('avg(rating) as rating'),
                    \Illuminate\Support\Facades\DB::raw('sum(amount) as sold_count'),
                ]);
